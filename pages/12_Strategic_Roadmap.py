# ==========================================================
# STRATEGIC ROADMAP
# Sprint 3D.1 - Production Version
# ==========================================================
...
selected_agency = st.selectbox(
    "Select Agency",
    sorted(
        benchmark_df["Agency"].dropna().unique()
    )
)

agency_row = benchmark_df[
    benchmark_df["Agency"] == selected_agency
].iloc[0]
...
st.markdown("## Agency Strategic Profile")

c1, c2, c3, c4, c5, c6 = st.columns(6)

c1.metric("DMI", round(agency_row["DMI"], 1))
c2.metric("FMI", round(agency_row["FMI"], 1))
c3.metric("RRI", round(agency_row["RRI"], 1))
c4.metric("DRI", round(agency_row["DRI"], 1))

c5.metric(
    "Overall Score",
    round(agency_row["Overall_Score"], 1)
)

c6.metric(
    "Rank",
    f"{int(agency_row['Overall_Rank'])} / {len(benchmark_df)}"
)
...
scores = {
    "Data Maturity": agency_row["DMI"],
    "Forecasting Maturity": agency_row["FMI"],
    "Reconstruction Readiness": agency_row["RRI"],
    "Digital Readiness": agency_row["DRI"]
}
...
roadmap = ROADMAP[weakest]

col1, col2, col3 = st.columns(3)

with col1:
    st.markdown("### Short Term")
    st.caption("0–12 Months")
...
st.markdown("## Maturity Improvement Path")

overall_score = agency_row["Overall_Score"]
...
path_df = pd.DataFrame({
    ...
    "Status": [
        "Current" if stage == maturity_stage else ""
        for stage in [
            "Emerging",
            "Developing",
            "Advanced",
...
**{int(agency_row['Overall_Rank'])} out of {len(benchmark_df)}**
participating organizations, with an overall score of
